...
function sdm_About()
	print( sdm_printPrefix .. 'by hypehuman and tweaked by spiralofhope. Version ' .. sdm_version .. '. Check for updates at https://github.com/spiralofhope/SuperDuperMacro/releases' )
end
...
function sdm_UpdateList()
	...
	for i,mTab in ipairs( sorted ) do
      ...
      if type( mTab.icon ) == 'number' then
		...
		listItem.icon:SetPoint(
      'LEFT',
      sdm_iconSize * ( sdm_iconSpacing + offsets[ i ] * ( sdm_iconSpacing + 1 ) ) + ( sdm_iconSize - listItem.icon:GetWidth() ) / 2,
      0
    )
		listItem.text:SetPoint(
      'LEFT',
      sdm_iconSize * ( sdm_iconSpacing + ( offsets[ i ] + 1 ) * ( sdm_iconSpacing + 1 ) ),
      0
    )
...
function sdm_ShowUsage( mTab )
	if sdm_UsedByThisChar( mTab ) then
        ...
        .. string.format( "%q", '/click sdb_' .. mTab.name )
        .. ' (case-sensitive).'
      )
        ...
        .. string.format( "%q", '/click sdf_' .. mTab.name )
        .. ' (case-sensitive).'
      )
        ...
        .. string.format("%q", '/sdm run ' .. mTab.name)
        .. ' or use the function sdm_RunScript('
        .. string.format( "%q", mTab.name )
        .. ') (case-sensitive).'
      )
		end
	else
